...
if (p == 'asp' || p == 'spx') { text = document.selection.createRange().text; }
else { text = window.getSelection().toString(); }
if ((document.activeElement.nodeName).toUpperCase() == 'INPUT') {
  text = getInput();
  }
if (!text || text=='') { Url = 'https://www.google.com/';window.open(Url, "_blank"); }
else {
text = checkSpaces(text);
Url = 'https://www.google.com/?gws_rd=ssl#safe=active&q=' + text;
